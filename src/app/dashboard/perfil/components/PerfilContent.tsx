...
export default function PerfilContent({ dadosPerfil, carregando, onSalvar }: PerfilContentProps) {
  ...
  const [redesAbertas, setRedesAbertas] = useState(false); // Estado da sanfona
      ...
      {carregando ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {/* BLOCO 2: REDES SOCIAIS EM SANFONA */}
<div className="bg-zinc-950/80 border border-zinc-800/80 rounded-xl p-6 shadow-xl">
  <div className="flex justify-between items-center cursor-pointer" onClick={() => setRedesAbertas(!redesAbertas)}>
    <div>
    ...
    >
      <svg 
        className={`w-4 h-4 transition-transform duration-300 ${redesAbertas ? "rotate-180" : ""}`} 
  ...
  {redesAbertas && (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-5 pt-5 border-t border-zinc-900 animate-in fade-in slide-in-from-top-2 duration-200">
      {[
        ...
        <div key={rede.id}>
          ...
          <div className="space-y-6">
            <div className="bg-zinc-950 border border-zinc-800/80 rounded-xl shadow-xl overflow-hidden text-center relative">
              ...
              {/* PREVIEW DAS REDES SOCIAIS (SÓ APARECE SE TIVER ALGO PREENCHIDO E AGORA SÃO CLICÁVEIS) */}
<div className="flex items-center justify-center gap-3 mb-4 relative z-10">
  {redesSociais.youtube && (
    <a 
      href={redesSociais.youtube.startsWith('http') ? redesSociais.youtube : `https://${redesSociais.youtube}`} 
      target="_blank" 
      rel="noopener noreferrer"
      className="w-6 h-6 bg-zinc-900 rounded-md flex items-center justify-center border border-zinc-800 text-red-500 hover:scale-110 transition-transform cursor-pointer" 
      title="YouTube"
    >
  ...
  {redesSociais.instagram && (
    <a 
      href={redesSociais.instagram.startsWith('http') ? redesSociais.instagram : `https://${redesSociais.instagram}`} 
      target="_blank" 
      rel="noopener noreferrer"
      className="w-6 h-6 bg-zinc-900 rounded-md flex items-center justify-center border border-zinc-800 text-pink-500 hover:scale-110 transition-transform cursor-pointer" 
      title="Instagram"
    >
  ...
  {redesSociais.tiktok && (
    <a 
      href={redesSociais.tiktok.startsWith('http') ? redesSociais.tiktok : `https://${redesSociais.tiktok}`} 
      target="_blank" 
      rel="noopener noreferrer"
      className="w-6 h-6 bg-zinc-900 rounded-md flex items-center justify-center border border-zinc-800 text-white hover:scale-110 transition-transform cursor-pointer" 
      title="TikTok"
    >
  ...
  {redesSociais.twitter && (
    <a 
      href={redesSociais.twitter.startsWith('http') ? redesSociais.twitter : `https://${redesSociais.twitter}`} 
      target="_blank" 
      rel="noopener noreferrer"
      className="w-6 h-6 bg-zinc-900 rounded-md flex items-center justify-center border border-zinc-800 text-white hover:scale-110 transition-transform cursor-pointer" 
      title="Twitter/X"
